#!/usr/bin/env python3
"""Build E02 from exact blobs; rewrite frozen E01 execution before helper replacement."""
...
def main() -> int:
    spec = importlib.util.spec_from_file_location("cmi_flu_strategy_e02_prepare_base_v6", BASE)
    ...
    def robust_patch_runtime(runtime: str, e02: str, e02_v2: str) -> str:
        # Rewrite the execution call before replacing helper definitions.  The
        # frozen template is allowed to keep legacy stage-label strings.
        for old, new in (
            ...
            if runtime.count(old) != 1:
                raise SystemExit(f"E02 prepatch execution token changed: {old} count={runtime.count(old)}")
