"""Validate the source-version-3 Stage 1 cached-continuation request."""
...
PATCHER_PATH = "scripts/poisoned_chalice_stage1_resume_v2_patch.py"
PATCHER_BLOB = "c816017d507f68c1a348b566efa257ecfbe7f6ee"
...
def expected_request() -> dict:
    return {
        ...
        "parent_failed_request_id": "20260903-poisoned-chalice-stage1-resume-001",
...
def validate_sources(request_path: Path, builder_path: Path, patcher_path: Path) -> dict:
    request = json.loads(request_path.read_text(encoding="utf-8"))
    if request != expected_request():
        raise RuntimeError("resume-v2 request differs from exact contract")
    builder = builder_path.read_bytes()
    patcher = patcher_path.read_bytes()
    if git_blob_sha(builder) != BUILDER_BLOB:
        raise RuntimeError("resume builder Git blob mismatch")
    if git_blob_sha(patcher) != PATCHER_BLOB:
        raise RuntimeError("resume provenance patcher Git blob mismatch")
    compile(builder, BUILDER_PATH, "exec")
    compile(patcher, PATCHER_PATH, "exec")
    return request
...
def validate_notebook(root: Path) -> dict:
    ...
    if files != ["kernel-metadata.json", "stage1-raw-fim-resume-v1.ipynb"]:
        raise RuntimeError(f"unexpected resume-v2 kernel files: {files}")
    ...
    if metadata != expected_metadata:
        raise RuntimeError("resume-v2 kernel metadata changed")
    notebook = json.loads(
        (root / metadata["code_file"]).read_text(encoding="utf-8")
    )
    if notebook.get("nbformat") != 4 or len(notebook.get("cells", [])) != 2:
        raise RuntimeError("resume-v2 notebook structure changed")
    if [cell.get("id") for cell in notebook["cells"]] != [
        "pc-stage1-resume-00",
        "pc-stage1-resume-01",
    ]:
        raise RuntimeError("resume-v2 stable cell IDs missing")
    ...
    for name, value in expected_constants.items():
        if constants.get(name) != value:
            raise RuntimeError(f"resume-v2 notebook constant changed: {name}")
    if "20260903-poisoned-chalice-stage1-resume-001" in code:
        raise RuntimeError("failed resume request ID remains in patched notebook")
    for marker in (
        ...
        if marker not in code:
            raise RuntimeError(f"resume-v2 notebook invariant missing: {marker}")
    forbidden_imports = {
    ...
    if imported.intersection(forbidden_imports):
        raise RuntimeError(
            f"resume-v2 notebook gained forbidden imports: {sorted(imported & forbidden_imports)}"
        )
    for forbidden in (
        ...
        if forbidden in code:
            raise RuntimeError(f"resume-v2 notebook gained forbidden capability: {forbidden}")
    return {
        "request_id": constants["REQUEST_ID"],
        "source_kernel_version": constants["SOURCE_KERNEL_VERSION"],
        "accelerator": "cpu",
        "internet": False,
...
def validate_launch(path: Path) -> None:
    ...
    for marker in (
        ...
        "requests/poisoned-chalice-stage1-resume-v2.json",
        ...
        "poisoned_chalice_stage1_resume_v2_patch.py",
        "RESUME_V2_LIVE_PREFLIGHT PASS",
        "SOURCE_DATASET_V2_SCHEMA PASS feature_shards=60",
        "STAGE1_RESUME_V2_KERNEL_BUILD PASS",
        "RESUME_V2_KERNEL_LAUNCH PASS",
        "automatic_retries=0 submission=false",
        "STAGE1_RESUME_V2_RUNNER_LOCAL_MATERIAL_REMOVED",
    ):
        if marker not in source:
            raise RuntimeError(f"resume-v2 launch invariant missing: {marker}")
    if source.count('"${WORKDIR}/venv/bin/kaggle" kernels push') != 1:
        raise RuntimeError("resume-v2 launch must contain exactly one kernels push")
    secret_marker = "$" + "{{ secrets.KAGGLE_API_TOKEN }}"
    if source.count(secret_marker) != 3:
        raise RuntimeError("resume-v2 Kaggle secret step count changed")
    for forbidden in (
        ...
        if forbidden in source:
            raise RuntimeError(f"resume-v2 launch gained forbidden operation: {forbidden}")
    if re.search(r"^\s*sleep\s+", source, flags=re.MULTILINE):
        raise RuntimeError("resume-v2 launch must not poll")
...
def validate(
    ...
    request = validate_sources(request_path, builder_path, patcher_path)
    with tempfile.TemporaryDirectory(prefix="stage1-resume-v2-") as temporary:
...
def main() -> None:
    ...
    validate(args.request, args.builder, args.patcher, args.launch_workflow)
