...
Repair 002 differs from recovery 001 only in one transport detail: the shared
current-output reader owns creation of the final output directory, so this
wrapper must not pre-create that directory.
"""
...
REQUEST_ID = "20260909-cmi-flu-strategy-e08-result-recovery-002"
...
def main() -> int:
    ...
    if out.exists():
        raise SystemExit("E08 recovery output directory must not pre-exist")
