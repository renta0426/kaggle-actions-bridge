"""Patch only immutable provenance for cached Stage 1 continuation request 003."""
...
NEW_REQUEST_ID = "20260903-poisoned-chalice-stage1-resume-003"
...
def patch(path: Path) -> dict[str, object]:
    ...
    if OLD_REQUEST_ID in source:
        raise RuntimeError("old request ID remains after provenance patch")
    if f"SOURCE_KERNEL_VERSION = {OLD_SOURCE_VERSION}" in source:
        raise RuntimeError("old source version remains after provenance patch")
