...
REQUEST_ID = "20260904-cmi-flu-rank-transfer-002"
...
def main() -> int:
    ...
    request_path = root / "requests/cmi-flu-rank-transfer-002.json"
    ...
    run(sys.executable, str(root / "scripts/cmi_flu_rank_transfer_patch_v2.py"), "--source", str(b21), "--rank-source", str(rank_path), "--request", str(request_path), "--output", str(output))
    ...
    print(
        "CMI_FLU_RANK_TRANSFER_002_PREPARE PASS "
        f"science_commit={SCIENCE_COMMIT} rank_blob={RANK_TRANSFER_BLOB}"
    )
    return 0
