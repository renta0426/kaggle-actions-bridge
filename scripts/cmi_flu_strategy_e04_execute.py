#!/usr/bin/env python3
"""One-shot private CPU executor for CMI-Flu E04 Task1.3."""
...
TARGET="renta0426/cmi-flu-e04-task13-rescue-20260908-001"
TARGET_SLUG=TARGET.split("/",1)[1]
TITLE="CMI Flu E04 Task13 Rescue 20260908 001"
EXPECTED_VERSION=1
POLL_SECONDS=60
MAX_POLLS=65
...
def prewrite_guard(api):
 ...
 if owner!='renta0426' or slug!=TARGET_SLUG or TITLE!='CMI Flu E04 Task13 Rescue 20260908 001':raise RuntimeError('E04 target identity contract changed')
 try: discovered=api.kernels_list(user=owner,search=TARGET_SLUG,page_size=20) or []
 except Exception as e:raise RuntimeError('E04 duplicate sentinel unavailable') from e
 if any(str(getattr(x,'ref',''))==TARGET for x in discovered):raise RuntimeError('E04 duplicate sentinel found exact target; write refused')
def live_rules(api):
 ...
 if 'evaluation' not in content or 'mean spearman correlation' not in plain(content['evaluation']):raise RuntimeError('E04 live evaluation guard changed')
...
def push(api,runtime,work):
 ...
 try:m=kernel_meta(api,TARGET)
  ...
  code=hashlib.sha256(f'{type(e).__name__}:{e}'.encode(errors='replace')).hexdigest()[:20]; print(f'CMI_FLU_E04_WRITE_AMBIGUOUS exception_type={type(e).__name__} error_code={code}'); raise RuntimeError('E04 push acknowledged but exact metadata unconfirmed') from e
 if str(getattr(m,'ref',''))!=TARGET or not bool(getattr(m,'is_private',False)) or bool(getattr(m,'enable_gpu',False)) or bool(getattr(m,'enable_tpu',False)) or bool(getattr(m,'enable_internet',False)):raise RuntimeError('pushed E04 metadata contract mismatch')
 if int(getattr(m,'current_version_number',0) or 0)!=1:raise RuntimeError('E04 direct version mismatch')
 receipt('exact_identity_confirmed',runtime,True,int(c.returncode),c.stdout,c.stderr,True)
def wait(api):
 for _ in range(MAX_POLLS):
  m=kernel_meta(api,TARGET)
  if int(getattr(m,'current_version_number',0) or 0)!=1:raise RuntimeError('E04 version changed during execution')
  status=str(getattr(api.kernels_status(TARGET),'status','')).upper()
  if 'COMPLETE' in status:return status
  if any(t in status for t in ('ERROR','CANCEL','FAIL')):raise RuntimeError('E04 remote failed')
  if not any(t in status for t in ('RUNNING','QUEUED','PENDING')):raise RuntimeError(f'unknown E04 status:{status}')
  time.sleep(POLL_SECONDS)
 raise RuntimeError('E04 polling bound exceeded')
def main():
 ...
 try:
  ...
  if int(getattr(m,'current_version_number',0) or 0)!=1:raise RuntimeError('E04 version changed before output read')
  reader=a.bridge_root.resolve()/'scripts'/'kaggle_current_output_read.py'; c=subprocess.run([sys.executable,str(reader),'--kernel',TARGET,'--expected-version','1','--allow-file','bridge-result.json:1048576','--allow-file','metrics.json:16777216','--allow-file','summary.md:1048576','--output-dir',str(a.output_dir.resolve())],timeout=240,check=False)
  if c.returncode:raise RuntimeError('E04 aggregate output read failed')
  if int(getattr(kernel_meta(api,TARGET),'current_version_number',0) or 0)!=1:raise RuntimeError('E04 version changed after output read')
  print(f'CMI_FLU_E04_REMOTE PASS status={status} version=1')
