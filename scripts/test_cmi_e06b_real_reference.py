#!/usr/bin/env python3
"""Verify E06b uses the exact organizer sequence reference through native-schema adapter."""
...
def load_runtime(path: Path):
    spec = importlib.util.spec_from_file_location("e06b_real_reference_runtime", path)
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="e06b-real-reference-") as tmp:
        package_path = Path(tmp) / "cmi_flu_bundle.zip"
        package_path.write_bytes(runtime.package_bytes())
        sys.path.insert(0, str(package_path))
        try:
            run, hai, e06bv2 = runtime.load_e06_module()
            assert callable(run) and callable(e06bv2.run_strategy_e06b)
    ...
    print(
        "CMI_FLU_E06B_REAL_REFERENCE_PASS hash_verified=true native_header=true "
