...
OUT_DIR = Path("notebooks/poisoned-chalice-stage2-deployment-analysis-v3-repair")
...
def main() -> int:
    analyzer_path = Path(sys.argv[1]) if len(sys.argv) > 1 else Path("scripts/poisoned_chalice_stage2_deployment_analysis_v3.py")
...
print("STAGE2_ANALYSIS_V3_INSTALL PASS")'''
    materialize = (
        "from pathlib import Path\n"
        "ANALYZER = Path('/tmp/poisoned_chalice_stage2_deployment_analysis_v3.py')\n"
        "ANALYZER.write_text(__SOURCE__, encoding='utf-8')\n"
        "compile(ANALYZER.read_text(encoding='utf-8'), str(ANALYZER), 'exec')\n"
        "print('STAGE2_ANALYSIS_V3_MATERIALIZE PASS')\n"
    ).replace("__SOURCE__", repr(analysis_source))
    run = '''import subprocess, sys
completed = subprocess.run([sys.executable, "/tmp/poisoned_chalice_stage2_deployment_analysis_v3.py"], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=3600)
print(completed.stdout)
if completed.returncode != 0:
    raise RuntimeError(f"STAGE2_DEPLOYMENT_ANALYSIS_V1_V3_FAILED rc={completed.returncode}")
print("STAGE2_DEPLOYMENT_ANALYSIS_V1_NOTEBOOK COMPLETE")'''
    notebook = {
        "cells": [
            markdown_cell(
                "# STAGE2-DEPLOYMENT-ANALYSIS-V1 — repair version 3\n\n"
                "CPU-only analysis of the exact successful Stage2 deployment output. "
                "Version 3 keeps evaluation.json authoritative for preregistered aggregate metrics, "
                "uses persisted scores for row-level diagnostics, and records the fusion rank/tie artifact discrepancy instead of tuning or failing on it.",
                "intro",
            ),
