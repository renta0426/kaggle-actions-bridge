...
def load_source(root: Path) -> str:
    ...
    required = (
        ...
        "OUTER_SPLITS = 5",
        "OUTER_REPEATS = 3",
        "def run_v3_05_task13(",
        "def write_v3_05_outputs(",
        '"not_executed_data_limited"',
...
def build_runtime(root: Path, output: Path) -> str:
    ...
    import sys, types
    # Loading V3-02 first installs the frozen-runtime evaluate_model_spec
    # compatibility shim. It changes no split, fit or prediction.
    load_v3_v02_module()
...
'''
    ...
    declared = [
        ...
        "v3_v05_task13_summary.json", "v3_v05_task13_bank_manifest.json",
    ]
    published = []
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="cmi-v305-", dir="/tmp") as tmp:
            ...
            if aggregate.get("new_candidate_conditions") != ["S1", "S2", "S3", "S4"]:
                raise BridgeContractError("v305_condition_set_changed")
            if int(aggregate.get("new_candidate_condition_count", -1)) != 4:
                raise BridgeContractError("v305_condition_count_changed")
            ...
            if sorted(p.name for p in staged.iterdir()) != sorted(declared):
                raise BridgeContractError("v305_output_allowlist_mismatch")
            if any(p.is_symlink() or not p.is_file() for p in staged.iterdir()):
                raise BridgeContractError("v305_nonregular_output")
            ...
            success = True
        print(
            "CMI_FLU_V3_V05_RUNTIME_PASS "
            f"fit_count={aggregate.get('fit_count')} oof_rows={len(oof_bank)} challenge_rows={len(challenge_bank)} "
            "new_conditions=4 task12=data_limited competition_submit=false public_used=false private_bank=true scratch_cleanup=true"
        )
        return 0
...
'''
    ...
    with tempfile.TemporaryDirectory(prefix="v305-selftest-") as tmp:
        ...
        try:
            ...
            if module.EXPERIMENT != "strategy_v3_v05_task13_absolute_scale_heads":
                raise BridgeContractError("v305_experiment_identity")
            if module.NEW_CONDITIONS != ("S1", "S2", "S3", "S4") or module.MAX_FITS != 96:
                raise BridgeContractError("v305_frozen_condition_contract")
        finally:
            if sys.path and sys.path[0] == str(package): sys.path.pop(0)
    print(
        f"CMI_FLU_V3_V05_SELF_TEST PASS request_id={V305_REQUEST_ID} science_commit={V305_SCIENCE_COMMIT} "
        f"blob={V305_SCIENCE_BLOB} conditions=4 fit_limit=96 submission=false"
    )
    return 0
'''
    ...
    try:
        ...
        required = (
            ...
            "sample_submission_part1.csv", "md5sum",
        )
...
def main() -> int:
    ...
    print(
        f"CMI_FLU_V3_V05_PREPARE PASS bytes={len(raw)} sha256={hashlib.sha256(raw).hexdigest()} "
        f"science_commit={SCIENCE_COMMIT} science_blob={SCIENCE_BLOB} conditions=4 submission=false"
    )
    return 0
