...
def _load_module(path: Path, expected_blob: str, name: str):
    data = path.read_bytes()
    if blob_sha(data) != expected_blob:
        raise RuntimeError(f"{name} Git blob mismatch")
...
def load_snapshots(root: Path) -> dict[str, bytes]:
    ...
    for logical, (bridge_path, expected_blob, maximum) in SNAPSHOTS.items():
        data = (root / bridge_path).read_bytes()
...
def _driver_source() -> str:
    return (
        ...
        "from poisoned_chalice.shadow_training_spawn import SHADOW_XLA_SPAWN_COMPAT_VERSION, train_shadow_model_spawn_safe\n"
        "def main():\n"
        ...
        "    print(json.dumps({'status': manifest['status'], 'slot': slot, 'completed_steps': manifest['completed_steps'], 'xla_compat': XLA_COMPAT, 'kaggle_tpu_env': KAGGLE_TPU_ENV, 'xla_spawn_compat': SHADOW_XLA_SPAWN_COMPAT_VERSION}, sort_keys=True))\n"
        "if __name__ == '__main__':\n"
        "    main()\n"
    )


def build_notebook_code(files: dict[str, bytes], v3_path: Path, v2_path: Path) -> str:
    v3 = _load_module(v3_path, V3_LAUNCHER_BLOB_SHA, "shadow_tpu_pilot_v3_builder")
    if blob_sha(v2_path.read_bytes()) != V2_LAUNCHER_BLOB_SHA:
    ...
    adapted = {k: v for k, v in files.items() if k != "shadow_tpu_pilot_v4.json"}
    adapted["shadow_tpu_pilot_v3.json"] = files["shadow_tpu_pilot_v4.json"]
    code = v3.build_notebook_code(adapted, v2_path)

    lines = code.splitlines()
    driver_replaced = import_replaced = init_injected = False
    for index, line in enumerate(lines):
        ...
        if line.strip() == '"from .shadow_xla_compat import install_shadow_xla_compat\\n"':
            ...
            init_injected = True
    if not (driver_replaced and import_replaced and init_injected):
        raise RuntimeError(
            f"pinned v3 builder structure changed: driver={driver_replaced} import={import_replaced} init={init_injected}"
        )
    code = "\n".join(lines) + "\n"

    for old, new, label in (
        ("if config.get('repair_from') != 'shadow-tpu-pilot-v2':", "if config.get('repair_from') != 'shadow-tpu-pilot-v3':", "repair parent"),
        ("    'repair_from': 'shadow-tpu-pilot-v2',", "    'repair_from': 'shadow-tpu-pilot-v3',", "summary parent"),
        ("shadow TPU compatibility pilot v3 failed acceptance gates; do not retry automatically", "shadow TPU compatibility pilot v4 failed acceptance gates; do not retry automatically", "failure marker"),
    ):
        if old not in code:
    ...
    if old_scope not in code:
        raise RuntimeError("pinned v3 builder repair scope not found")
    code = code.replace(old_scope, new_scope, 1)
    anchor = "    'xla_compat': XLA_COMPAT,"
    if anchor not in code:
        raise RuntimeError("summary XLA marker not found")
    code = code.replace(anchor, anchor + "\n    'kaggle_tpu_env': KAGGLE_TPU_ENV,", 1)
...
def validate_bundle(kernel_dir: Path) -> None:
    ...
    notebook = json.loads((kernel_dir / NOTEBOOK_NAME).read_text())
    ...
    for marker in (
        ...
        "shadow-xla-compat-v1", "shadow-xla-spawn-safe-v1", "shadow-kaggle-tpu-pjrt-env-v1",
        "sanitize_kaggle_tpu_pjrt_environment", "TPU_PROCESS_ADDRESSES", "CLOUD_TPU_TASK_ID",
        "train_shadow_model_spawn_safe", "if __name__ == '__main__':", SUMMARY_NAME, "PJRT_DEVICE",
    ):
    ...
    if code.index("sanitize_kaggle_tpu_pjrt_environment()") > code.index("install_shadow_xla_compat()"):
        raise RuntimeError("Kaggle TPU environment sanitizer must precede torch_xla import")
    for forbidden in (
        "KAGGLE_API_TOKEN", "RESEARCH_REPO_READ_TOKEN",
        ...
        "competitions submit", "HuggingFaceTB/SmolLM2",
    ):
...
def materialize(snapshot_root: Path, v3_path: Path, v2_path: Path, kernel_dir: Path) -> None:
    ...
    if config.get("experiment_id") != EXPERIMENT_ID or config.get("repair_from") != "shadow-tpu-pilot-v3":
        raise RuntimeError("pilot identity or repair parent changed")
    if config.get("repair_scope") != {
    ...
    code = build_notebook_code(files, v3_path, v2_path)
    notebook = {
        "cells": [
            {"cell_type": "markdown", "id": "shadow-tpu-v4-intro", "metadata": {}, "source": ["# Controlled shadow TPU compatibility pilot v4\n", "Synthetic compatibility-only re-test after one scoped Kaggle PJRT topology-environment repair.\n"]},
            {"cell_type": "code", "id": "shadow-tpu-v4-run", "execution_count": None, "metadata": {}, "outputs": [], "source": code.splitlines(keepends=True)},
        ],
    ...
    metadata = {
        "id": TARGET, "title": "Controlled shadow TPU compatibility pilot v4",
        "code_file": NOTEBOOK_NAME, "language": "python", "kernel_type": "notebook",
        "is_private": True, "enable_gpu": False, "enable_tpu": True, "enable_internet": False,
    ...
    print("SHADOW_TPU_V4_MATERIALIZE PASS snapshots=6 private_repo_access=0 spawn_safe=1 topology_env_sanitized=1")
...
def validate_static(launcher: Path, v3_path: Path, v2_path: Path) -> None:
    source = launcher.read_text()
    compile(source, str(launcher), "exec")
    _load_module(v3_path, V3_LAUNCHER_BLOB_SHA, "shadow_tpu_pilot_v3_builder")
    if blob_sha(v2_path.read_bytes()) != V2_LAUNCHER_BLOB_SHA:
        raise RuntimeError("v2 foundation launcher Git blob mismatch")
    commands = _literal_commands(source)
    def has_pair(command, left, right):
        return any(command[i:i + 2] == (left, right) for i in range(len(command) - 1))
    if sum(has_pair(c, "kernels", "push") for c in commands) != 1:
        raise RuntimeError("launcher must contain exactly one kernels push")
    for pair in (("competitions", "submit"), ("datasets", "create"), ("datasets", "version"), ("models", "create"), ("kernels", "delete"), ("kernels", "cancel")):
        if any(has_pair(c, *pair) for c in commands):
            raise RuntimeError(f"launcher gained forbidden write: {' '.join(pair)}")


def execute(kaggle_bin: Path, kernel_dir: Path) -> None:
    validate_bundle(kernel_dir)
    result = subprocess.run([str(kaggle_bin), "kernels", "push", "-p", str(kernel_dir), "--timeout", "3600"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, check=False)
    if result.returncode != 0:
        raise RuntimeError(f"Kaggle push returned nonzero ({result.returncode}); outcome ambiguous and retry forbidden")
    print("SHADOW_TPU_V4_LAUNCH_ACCEPTED target=renta0426/shadow-tpu-pilot-v4 accelerator=tpu retries=0 submissions=0 private_repo_access=0 post_push_getkernel=0")
...
def main() -> None:
    ...
    if args.static:
        if None in (args.launcher, args.v3_launcher, args.v2_launcher):
        ...
        print("SHADOW_TPU_V4_STATIC PASS write_calls=1 retries=0 submissions=0 private_repo_access=0")
        return
    if args.materialize:
        if None in (args.snapshot_root, args.v3_launcher, args.v2_launcher, args.kernel_dir):
            raise SystemExit("--snapshot-root, --v3-launcher, --v2-launcher and --kernel-dir required")
        materialize(args.snapshot_root, args.v3_launcher, args.v2_launcher, args.kernel_dir)
        return
