#!/usr/bin/env python3
"""Aggregate-only sanitizer for E05 007 frozen-evaluation compatibility."""
...
def main() -> int:
    ...
    if files != ["bridge-result.json", "metrics.json", "summary.md"]:
        raise SystemExit(f"E05 v6 safe-output file set mismatch:{files}")
    texts = {name: (root / name).read_text() for name in files}
    for name, text in texts.items():
        if any(token in text for token in BANNED):
            raise SystemExit(f"E05 v6 privacy contract failed:{name}")
    ...
    expected = {
        ...
        "strategy_e05_v4_blob_sha": E05_V4_BLOB,
    ...
    if fc.get("ridge_alpha") != 10.0 or fc.get("interaction_count") != 8 or fc.get("free_participant_embedding") is not False or fc.get("raw_strain_id_one_hot") is not False:
        raise SystemExit("E05 v6 feature contract mismatch")
    if fc.get("donor_rank_unit") != "participant_id_within_study":
        raise SystemExit("E05 v6 donor rank unit mismatch")
    if fc.get("subject_group_role") != "leakage_purge_and_hierarchical_weighting_only":
        raise SystemExit("E05 v6 subject-group role mismatch")
    if fc.get("sequence_reference_schema") != "organizer_native_or_canonical_explicit_adapter":
        raise SystemExit("E05 v6 sequence reference schema mismatch")
    if fc.get("organizer_sequence_reference_columns") != ["Virus", "Sequence", "Status_of_sequence"]:
        raise SystemExit("E05 v6 organizer sequence columns mismatch")
    if fc.get("legacy_evaluation_without_fold_fields_supported") is not True:
        raise SystemExit("E05 v6 legacy evaluation compatibility missing")
    if fc.get("panel_proxy_fold_metrics_contract") != "recomputed_from_oof_split_participant_panel":
        raise SystemExit("E05 v6 panel fold contract mismatch")
    if sc.get("fixed_subject_strain_simultaneous_holdouts") != 4 or sc.get("simultaneous_selection_uses_outcomes") is not False:
        raise SystemExit("E05 v6 split contract mismatch")
    if wc.get("rows_are_not_counted_as_independent_donors") is not True:
        raise SystemExit("E05 v6 weight contract mismatch")
    ...
    for task, payload in metrics["tasks"].items():
        ...
        if not finite_or_none(delta):
            raise SystemExit(f"E05 v6 nonfinite route delta:{task}")
        if set(payload.get("study_out_conditions") or {}) != expected_conditions:
            raise SystemExit(f"E05 v6 condition set mismatch:{task}")
        print(
    ...
    print(
        "CMI_FLU_E05_V6_RESULT PASS aggregate_only=true submission=false "
        f"science_commit={SCIENCE_COMMIT} e05_v4_blob={E05_V4_BLOB}"
    )
    return 0
