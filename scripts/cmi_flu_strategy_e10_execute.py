#!/usr/bin/env python3
"""One-shot private CPU executor for CMI-Flu E10 pooled domain correction."""
...
for key, value in _EXPECTED_PRIOR.items():
    if getattr(prior, key, None) != value:
        raise SystemExit(f"E10 E06a-v3 executor ancestry changed:{key}")
if prior.prior.prior.POLL_SECONDS != 120 or prior.prior.prior.MAX_POLLS != 65:
    raise SystemExit("E10 watcher horizon contract changed")
base = prior.base
if base.EXPECTED_VERSION != 1:
    raise SystemExit("E10 executor version contract changed")
if "exact_metadata_eventually" not in base.kernel_meta.__code__.co_names:
    raise SystemExit("E10 executor lacks bounded exact metadata reconciliation")
...
def prewrite_guard(api):
    ...
    if owner != "renta0426" or slug != base.TARGET_SLUG or TITLE != "CMI Flu E10 Pooled Domain Correction 20260909 001":
        raise RuntimeError("E10 target identity contract changed")
    try:
        discovered = api.kernels_list(user=owner, search=base.TARGET_SLUG, page_size=20) or []
    except Exception as error:
        raise RuntimeError("E10 duplicate sentinel unavailable") from error
    if any(str(getattr(item, "ref", "")) == TARGET for item in discovered):
        raise RuntimeError("E10 duplicate sentinel found exact target; write refused")
