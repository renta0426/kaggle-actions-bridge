#!/usr/bin/env python3
"""Aggregate-only sanitizer for E05 004 donor-rank correction."""
...
def main() -> int:
    ...
    if files != ["bridge-result.json", "metrics.json", "summary.md"]:
        raise SystemExit(f"E05 v4 safe-output file set mismatch:{files}")
    texts = {name: (root / name).read_text() for name in files}
    for name, text in texts.items():
        if any(token in text for token in BANNED):
            raise SystemExit(f"E05 v4 privacy contract failed:{name}")
    ...
    for task, payload in metrics["tasks"].items():
        ...
        if not finite_or_none(delta):
            raise SystemExit(f"E05 v4 nonfinite route delta:{task}")
        if set(payload.get("study_out_conditions") or {}) != expected_conditions:
            raise SystemExit(f"E05 v4 condition set mismatch:{task}")
        print(
    ...
    print(
        "CMI_FLU_E05_V4_RESULT PASS aggregate_only=true submission=false "
        f"science_commit={SCIENCE_COMMIT} e05_blob={E05_BLOB} e05_v2_blob={E05_V2_BLOB}"
    )
    return 0
