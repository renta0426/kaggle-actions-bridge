...
def main() -> int:
    ...
    if files != ["bridge-result.json", "metrics.json", "summary.md"]:
        raise SystemExit(f"E05 v2 safe-output file set mismatch:{files}")
    texts = {name: (root / name).read_text() for name in files}
    for name, text in texts.items():
        if any(token in text for token in BANNED):
            raise SystemExit(f"E05 v2 privacy contract failed:{name}")
    ...
    for key, value in expected.items():
        if bridge.get(key) != value:
            raise SystemExit(f"E05 v2 bridge contract mismatch:{key}:{bridge.get(key)!r}")
    if bridge.get("strategy_e05_blob_sha") != E05_BLOB:
        raise SystemExit("E05 v2 bridge E05 blob mismatch")
    if bridge.get("metrics_sha256") != sha(root / "metrics.json") or bridge.get("summary_sha256") != sha(root / "summary.md"):
        raise SystemExit("E05 v2 output hash mismatch")

    if metrics.get("experiment") != "strategy_v2_e05_hai_donor_strain" or set((metrics.get("tasks") or {})) != {"Task2.1", "Task2.2"}:
        raise SystemExit("E05 v2 experiment/task identity mismatch")
    ...
    for task, payload in metrics["tasks"].items():
        ...
        if not finite_or_none(delta):
            raise SystemExit(f"E05 v2 nonfinite route delta:{task}")
        conditions = payload.get("study_out_conditions") or {}
        expected_conditions = {
        ...
        if set(conditions) != expected_conditions:
            raise SystemExit(f"E05 v2 condition set mismatch:{task}")
        print(
    ...
    print(f"CMI_FLU_E05_V2_RESULT PASS aggregate_only=true submission=false science_commit={SCIENCE_COMMIT} e05_blob={E05_BLOB}")
    return 0
