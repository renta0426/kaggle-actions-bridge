#!/usr/bin/env python3
"""E11b 003 executor: preserve science, replace opaque CLI push with direct KaggleApi.kernels_push."""
...
REQUEST_ID = "20260910-cmi-flu-strategy-e11b-tabpfn3-003"
TARGET = "renta0426/cmi-flu-e11b-tabpfn3-20260910-003"
TARGET_SLUG = TARGET.split("/", 1)[1]
TITLE = "CMI Flu E11b TabPFN3 20260910 003"
...
for key, value in _EXPECTED_PRIOR.items():
    if getattr(prior, key, None) != value:
        raise SystemExit(f"E11b executor-v3 ancestry changed:{key}")
...
def _safe_server_message(value: str) -> str | None:
    """Return only a bounded metadata/transport error safe for the public Actions log."""
    if not value:
        return None
...
def model_access_preflight() -> None:
    completed = subprocess.run(
        [
            ...
            prior.MODEL_SOURCE, "--format", "json", "--page-size", "20",
        ],
        capture_output=True,
        text=True,
        timeout=120,
    ...
    if completed.returncode != 0:
        raise RuntimeError("E11b 003 model access preflight failed; no kernel write attempted")
    try:
        payload = json.loads(stdout)
    except Exception as exc:
        raise RuntimeError("E11b 003 model access preflight returned non-JSON") from exc
    serialized = json.dumps(payload, sort_keys=True)
    if prior.CHECKPOINT_FILENAME not in serialized:
        raise RuntimeError("E11b 003 model access checkpoint not visible")
    if str(prior.CHECKPOINT_BYTES) not in serialized and f"{prior.CHECKPOINT_BYTES}.0" not in serialized:
        raise RuntimeError("E11b 003 checkpoint byte identity changed")
    print(
...
def prewrite_guard(api: KaggleApi) -> None:
    ...
    if owner != "renta0426" or slug != TARGET_SLUG or TITLE != "CMI Flu E11b TabPFN3 20260910 003":
        raise RuntimeError("E11b 003 target identity contract changed")
    ...
    if any(str(getattr(item, "ref", "")) == TARGET for item in discovered):
        raise RuntimeError("E11b 003 exact target already exists; write refused")
...
def push(api: KaggleApi, runtime: Path, work: Path) -> None:
    ...
    if {p.name for p in kernel_dir.iterdir()} != {"script.py", "kernel-metadata.json"}:
        raise RuntimeError("E11b 003 kernel payload file set changed")
    materialized = json.loads((kernel_dir / "kernel-metadata.json").read_text())
    if materialized != metadata:
        raise RuntimeError("E11b 003 materialized metadata changed")
    ...
    try:
        ...
        raise RuntimeError("E11b 003 direct SaveKernel failed; no automatic retry") from exc
    ...
    print("CMI_FLU_E11B_API_WRITE_RECEIPT " + json.dumps(response_summary, sort_keys=True))
    if error:
        safe = _safe_server_message(error)
        if safe:
            print("CMI_FLU_E11B_API_SAFE_ERROR message=" + safe)
        raise RuntimeError("E11b 003 SaveKernel response contained error; no automatic retry")
    ...
    if (
        ...
        raise RuntimeError("E11b 003 direct metadata identity mismatch")
    ...
    if prior.MODEL_SOURCE not in model_sources or prior.COMPETITION not in competition_sources:
        raise RuntimeError("E11b 003 source attachment mismatch")
    print(
        "CMI_FLU_E11B_API_IDENTITY_CONFIRMED "
        "version=1 private=true cpu=true model_attached=true competition_attached=true"
    )
