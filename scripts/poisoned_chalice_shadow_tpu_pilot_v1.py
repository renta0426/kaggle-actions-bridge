...
def validate_request(request: dict, launcher_path: Path) -> None:
    expected_keys = {
    ...
    if set(request) != expected_keys:
        raise RuntimeError("shadow TPU pilot request field allowlist changed")
    exact = {
        "schema_version": 1,
        ...
        "target": TARGET,
        "launcher_path": "scripts/poisoned_chalice_shadow_tpu_pilot_v1.py",
    ...
    if blob_sha(launcher_path.read_bytes()) != expected_launcher_blob:
        raise RuntimeError("shadow TPU pilot launcher source blob mismatch")
...
metadata_columns = ["benchmark_id", "exposure_round", "sequence_index", "source_token_count", "window_start", "window_name", "selected_payload_tokens"]
with metadata_path.open("w", encoding="utf-8", newline="\\n") as handle:
    for row in sequences[metadata_columns].itertuples(index=False, name=None):
