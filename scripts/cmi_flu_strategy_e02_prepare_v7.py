#!/usr/bin/env python3
"""Build E02 from exact blobs while preserving the frozen E01 runtime main body."""
...
def _insert_before_once(text: str, marker: str, addition: str, label: str) -> str:
...
def main() -> int:
    spec = importlib.util.spec_from_file_location("cmi_flu_strategy_e02_prepare_base_v7", BASE)
    ...
    def safe_patch_runtime(runtime: str, e02: str, e02_v2: str) -> str:
        # Patch the execution call before adding any helper definitions.  Keep
        # legacy E01 stage labels: they are diagnostic labels only and changing
        # them is not required for the E02 science contract.
        runtime = _replace_once(
            runtime,
            "run_e01 = load_e01_module()",
            "run_e02 = load_e02_module()",
            "E02 frozen execution loader token changed",
        )
        runtime = _replace_once(
            runtime,
            "result = json_safe(dict(run_e01(config, inputs)))",
            "result = json_safe(dict(run_e02(config, inputs)))",
            "E02 frozen execution call token changed",
        )
        ...
        runtime = _replace_once(
            ...
            install_anchor,
            compat,
            "E02 runtime install anchor changed",
        )

        source_marker = "REFERENCE_PLACEHOLDERS = "
        injected = (
    ...
    module.patch_runtime = safe_patch_runtime
