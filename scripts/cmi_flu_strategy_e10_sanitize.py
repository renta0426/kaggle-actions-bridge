#!/usr/bin/env python3
"""Validate aggregate-only E10 pooled domain-correction outputs."""
...
def validate_weight_audit(audit: dict, label: str) -> None:
    ...
    if min(int(value) for value in counts.values()) < 8:
...
def main() -> int:
    ...
    if frozen.get("source_weight_clip") != [0.5, 2.0] or frozen.get("deviation_scale") != 0.25 or frozen.get("minimum_source_subjects") != 8:
