#!/usr/bin/env python3
"""One-shot private CPU executor for CMI-Flu E07 Task1.4 formal closure."""
...
TITLE = "CMI Flu E07 Task14 Formal Closure 20260909 001"
EXPECTED_VERSION = 1
POLL_SECONDS = 60
MAX_POLLS = 35
...
def prewrite_guard(api: KaggleApi) -> None:
    ...
    if owner != "renta0426" or slug != TARGET_SLUG or TITLE != "CMI Flu E07 Task14 Formal Closure 20260909 001":
        raise RuntimeError("E07 target identity contract changed")
    try:
        discovered = api.kernels_list(user=owner, search=TARGET_SLUG, page_size=20) or []
    except Exception as exc:
        raise RuntimeError("E07 duplicate sentinel unavailable") from exc
    if any(str(getattr(item, "ref", "")) == TARGET for item in discovered):
        raise RuntimeError("E07 duplicate sentinel found exact target; write refused")
...
def live_rules(api: KaggleApi) -> None:
    ...
    if "evaluation" not in content or "mean spearman correlation" not in plain(content["evaluation"]):
        raise RuntimeError("E07 live evaluation guard changed")
...
def receipt(phase: str, runtime: Path, attempted: bool, rc: int | None = None, stdout: str = "", stderr: str = "", confirmed: bool = False) -> None:
    ...
    print("CMI_FLU_E07_WRITE_RECEIPT " + json.dumps(payload, sort_keys=True))
...
def push(api: KaggleApi, runtime: Path, work: Path) -> None:
    ...
    try:
        ...
        print(f"CMI_FLU_E07_WRITE_AMBIGUOUS exception_type={type(exc).__name__} error_code={code}")
        raise RuntimeError("E07 push acknowledged but exact metadata unconfirmed") from exc
    if (
        ...
        raise RuntimeError("pushed E07 metadata contract mismatch")
    if int(getattr(meta, "current_version_number", 0) or 0) != EXPECTED_VERSION:
        raise RuntimeError("E07 direct version mismatch")
...
def wait(api: KaggleApi) -> str:
    for _ in range(MAX_POLLS):
        meta = kernel_meta(api, TARGET)
        if int(getattr(meta, "current_version_number", 0) or 0) != EXPECTED_VERSION:
            raise RuntimeError("E07 version changed during execution")
        status = str(getattr(api.kernels_status(TARGET), "status", "")).upper()
        if "COMPLETE" in status:
            return status
        if any(token in status for token in ("ERROR", "CANCEL", "FAIL")):
            raise RuntimeError("E07 remote failed")
        if not any(token in status for token in ("RUNNING", "QUEUED", "PENDING")):
            raise RuntimeError(f"unknown E07 status:{status}")
        time.sleep(POLL_SECONDS)
    raise RuntimeError("E07 polling bound exceeded")
...
def main() -> int:
    ...
    try:
        ...
        meta = kernel_meta(api, TARGET)
        if int(getattr(meta, "current_version_number", 0) or 0) != EXPECTED_VERSION:
            raise RuntimeError("E07 version changed before output read")
        ...
        if completed.returncode:
            raise RuntimeError("E07 aggregate output read failed")
        if int(getattr(kernel_meta(api, TARGET), "current_version_number", 0) or 0) != EXPECTED_VERSION:
            raise RuntimeError("E07 version changed after output read")
        print(f"CMI_FLU_E07_REMOTE PASS status={status} version={EXPECTED_VERSION}")
    finally:
