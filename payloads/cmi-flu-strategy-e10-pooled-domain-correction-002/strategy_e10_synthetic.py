...
def run_synthetic() -> dict:
    ...
    minimum_source_training = pd.concat(
        [
            _frame("A", 5, shift=0.0),
            _frame("B", 8, shift=1.0),
        ],
        ignore_index=True,
    )
    minimum_source_weights = xonly_source_weights(
        minimum_source_training,
        target,
        assay_prefix="flow_",
    )
    row_weight = np.array(
    ...
    return {
        ...
        "minimum_source_counts": minimum_source_weights["source_counts"],
        "minimum_source_contract_supported": bool(
            minimum_source_weights["source_counts"] == {"A": 5, "B": 8}
            and abs(float(minimum_source_weights["row_weight_mean"]) - 1.0) < 1e-8
        ),
