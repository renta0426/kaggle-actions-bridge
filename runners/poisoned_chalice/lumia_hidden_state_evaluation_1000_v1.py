#!/usr/bin/env python3
"""One-shot launcher for the frozen P1-03 1k LUMIA hidden-state OOF evaluation."""
...
REQUEST_ID = "20260910-poisoned-chalice-lumia-hidden-state-eval-1000-v1-001"
...
RESEARCH_COMMIT = "2f531d7d4adb95cad87ad5eca603469e9bf5c19e"
...
OUTPUT_PREFIX = "lumia_hidden_state_evaluation_1000_v1"
PERSISTENT_OUTPUTS = [
...
def validate_request(request: dict) -> None:
    exact = {
        ...
        "target": TARGET,
        "launcher_path": "runners/poisoned_chalice/lumia_hidden_state_evaluation_1000_v1.py",
    ...
    for key, value in exact.items():
        if request.get(key) != value:
            raise RuntimeError(f"LUMIA evaluation 1k request changed: {key}")
...
def validate_research(root: Path) -> None:
    paths = {
        ...
        "builder": root / "scripts/build_lumia_hidden_state_evaluation_1000_notebook_v1.py",
    ...
    for marker in (
        ...
        if marker not in builder:
            raise RuntimeError(f"evaluation builder marker missing: {marker}")
...
def validate_kernel(root: Path) -> None:
    ...
    for marker in (
        ...
        "LUMIA_HIDDEN_STATE_EVAL_1000_V1 COMPLETE", "outer_oof_coverage",
        "caller_auc_plus_0_015", "helper_language_auc_bootstrap_lower_bound_positive",
    ):
        if marker not in code:
            raise RuntimeError(f"generated evaluation notebook marker missing: {marker}")
    for forbidden in ("competition_submit(", "submission.csv", "kaggle competitions submit"):
...
def materialize(request_path: Path, research_root: Path, kernel_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="lumia-eval-1000-nbformat-") as temp:
        shim = Path(temp)
        write_nbformat_shim(shim)
        env = os.environ.copy()
        env["PYTHONPATH"] = str(shim) + (os.pathsep + env["PYTHONPATH"] if env.get("PYTHONPATH") else "")
        process = subprocess.run(
            [sys.executable, str(research_root / "scripts/build_lumia_hidden_state_evaluation_1000_notebook_v1.py")],
    ...
    if process.returncode != 0:
        ...
        raise RuntimeError(
            f"1k evaluation notebook builder failed rc={process.returncode} "
            f"diagnostic_sha256={hashlib.sha256(diagnostic).hexdigest()}"
        )
    built = research_root / "notebooks/experiments/poisoned-chalice-lumia-hidden-state-eval-1000-v1"
    if kernel_dir.exists():
        shutil.rmtree(kernel_dir)
    shutil.copytree(built, kernel_dir)
    validate_kernel(kernel_dir)
    print("LUMIA_HIDDEN_STATE_EVAL_1000_V1_MATERIALIZE PASS rows=1000 outer_folds=5 layers=30 source_cache=kernel_input labels=public_train_only submissions=0")
...
def execute(request_path: Path, kernel_dir: Path, kaggle_bin: Path) -> None:
    ...
    if process.returncode:
        print(
            f"LUMIA_HIDDEN_STATE_EVAL_1000_V1_AMBIGUOUS_WRITE rc={process.returncode} "
    ...
    print(
        f"LUMIA_HIDDEN_STATE_EVAL_1000_V1_LAUNCH_ACCEPTED target={TARGET} source_cache={CACHE_KERNEL} "
        f"stdout_bytes={len(stdout)} stderr_bytes={len(stderr)} "
        f"stdout_sha256={hashlib.sha256(stdout).hexdigest()} "
...
def main() -> int:
    ...
    if args.static:
        ...
        print("LUMIA_HIDDEN_STATE_EVAL_1000_V1_STATIC PASS rows=1000 outer_folds=5 layers=30 bootstrap=1000")
        return 0
    if args.materialize:
