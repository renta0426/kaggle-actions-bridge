#!/usr/bin/env python3
"""Validate, materialize, and one-shot launch P1-03 LUMIA v4 target-site runtime gate."""
...
def validate_request(request: dict) -> None:
    ...
    if request.get("scientific_contract") != EXPECTED_SCIENCE:
        raise RuntimeError("LUMIA v4 scientific contract changed")
...
def validate_research(research_root: Path) -> None:
    ...
    if (
        repair.get("python_venv_used") is not False
        or repair.get("ensurepip_used") is not False
        or repair.get("live_kaggle_kernel_site_packages_mutated") is not False
        or repair.get("fresh_child_python") is not True
        or repair.get("pip_target_directory") != "/tmp/lumia-runtime-v4-site"
    ):
        raise RuntimeError("LUMIA v4 target-site repair changed")

    science = contract.get("scientific_contract_unchanged") or {}
    for key, value in EXPECTED_SCIENCE.items():
        if key in ("automatic_promotion", "competition_submission"):
            continue
        if science.get(key) != value:
            raise RuntimeError(f"LUMIA v4 frozen science changed: {key}")
    if science.get("validation_rows_used") is not False:
        raise RuntimeError("LUMIA v4 validation boundary changed")
