...
def fetch_exact(path: str, maximum: int = 262_144) -> bytes:
    url = f"https://raw.githubusercontent.com/{RESEARCH_REPO}/{RESEARCH_COMMIT}/{path}"
    parsed = urllib.parse.urlparse(url) if False else None
    request = urllib.request.Request(url, headers={"User-Agent": "poisoned-chalice-p1-03-post5k/1"})
    ...
    if not data or len(data) > maximum:
        raise RuntimeError(f"research source byte budget failed: {path}")
    expected = EXPECTED_BLOBS.get(path)
    if expected is None or git_blob_sha(data) != expected:
...
def materialize(request: dict, kernel_dir: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="p1-03-post5k-") as td:
        ...
        completed = subprocess.run([sys.executable, str(builder)], cwd=str(research), env=env, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=45)
        if completed.returncode != 0:
            raise RuntimeError(f"Notebook materialization failed rc={completed.returncode} output_sha256={sha256(completed.stdout.encode())}")
    ...
    print("P1_03_POST_5K_MATERIALIZE PASS cpu=1 retries=0 submissions=0")
...
def execute(request: dict, kernel_dir: Path, kaggle_bin: str) -> None:
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"one-shot SaveKernel failed rc={completed.returncode} stdout_sha256={sha256(stdout.encode())} stderr_sha256={sha256(stderr.encode())}")
    print(f"ONE_SHOT_WRITE_ATTEMPTED true target={TARGET} version=1 accelerator=cpu automatic_retries=0 submissions=0 stdout_sha256={sha256(stdout.encode())} stderr_sha256={sha256(stderr.encode())}")
...
def main() -> int:
    ...
    if args.static:
        ...
        print("P1_03_POST_5K_STATIC PASS science_commit=" + RESEARCH_COMMIT + " cpu=1")
        return 0
