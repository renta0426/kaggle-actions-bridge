#!/usr/bin/env python3
# Validate, materialize, and one-shot launch P1-03 LUMIA v3 isolated runtime gate.
...
REQUEST_ID = "20260909-poisoned-chalice-lumia-hs-runtime-50-v3-001"
TARGET = "renta0426/poisoned-chalice-lumia-hs-runtime-50-v3"
RESEARCH_COMMIT = "ccf87dd8ac51d5f331b716325c2e9c6261c86212"
...
def validate_request(request: dict) -> None:
    ...
    for key, value in exact.items():
        if request.get(key) != value:
            raise RuntimeError(f"LUMIA v3 request changed: {key}")
    if request.get("resource") != EXPECTED_RESOURCE:
        raise RuntimeError("LUMIA v3 resource contract changed")
    if request.get("api_budget") != {"max_calls": 60}:
        raise RuntimeError("LUMIA v3 API budget changed")
    if request.get("clean_room") != EXPECTED_CLEAN_ROOM:
        raise RuntimeError("LUMIA v3 clean-room contract changed")
    if request.get("scientific_contract") != EXPECTED_SCIENCE:
        raise RuntimeError("LUMIA v3 scientific contract changed")
...
def validate_research(research_root: Path) -> None:
    paths = {
        ...
        "builder": research_root / "scripts/build_lumia_hidden_state_runtime_pilot_notebook_v3.py",
        "contract": research_root / "configs/p1_03_lumia_hidden_state_runtime_pilot_50_v3_20260909.json",
    }
    ...
    for marker in (
        ...
        if marker not in core:
            raise RuntimeError(f"LUMIA v3 core marker missing: {marker}")
    for marker in (
    ...
    for marker in (
        ...
        "LUMIA_HIDDEN_STATE_RUNTIME_PILOT_V3 COMPLETE",
    ):
        if marker not in builder:
            raise RuntimeError(f"LUMIA v3 builder marker missing: {marker}")
    for forbidden in (
        "roc_auc_score",
        ...
        if forbidden in builder or forbidden in core:
            raise RuntimeError(f"performance/submission code forbidden in LUMIA v3: {forbidden}")

    if contract.get("task_id") != "P1-03-lumia-hidden-state-runtime-pilot-50-v1":
        raise RuntimeError("LUMIA v3 task identity changed")
    if contract.get("operational_attempt") != "v3_isolated_runtime_repair":
        raise RuntimeError("LUMIA v3 operational identity changed")
    if contract.get("status") != "implementation_pending_execution":
        raise RuntimeError("LUMIA v3 status changed")
    if contract.get("new_target") != TARGET:
        raise RuntimeError("LUMIA v3 target changed")
    if contract.get("failed_v2_target") != "renta0426/poisoned-chalice-lumia-hs-runtime-50-v2":
        raise RuntimeError("LUMIA v3 failed target changed")
    if contract.get("failed_v2_current_version") != 1:
        raise RuntimeError("LUMIA v3 failed version changed")
    if (contract.get("root_cause") or {}).get("classification") != "in_process_binary_abi_mutation":
        raise RuntimeError("LUMIA v3 root cause changed")
    repair = contract.get("repair") or {}
    if (
        repair.get("live_kaggle_kernel_pip_mutation") is not False
        or repair.get("isolated_venv") is not True
        or repair.get("child_python_process_for_all_data_model_and_scoring_work") is not True
    ):
        raise RuntimeError("LUMIA v3 isolation repair changed")
    if contract.get("scientific_contract_unchanged") != {
        key: EXPECTED_SCIENCE[key]
        for key in (
            "rows",
            "rows_per_language_label",
            "selection_seed",
            "selection_labels_removed_before_model_scoring",
            "model_id",
            "model_revision",
            "dataset_revision",
            "author_commit",
            ...
            "pooling_outputs_checked",
            "same_forward_output_scores_finite_only",
            "raw_activations_persisted",
            "sample_scores_persisted",
            "performance_metrics_computed",
            "direct_1000_gate_estimated_wall_minutes_lte",
        )
    } | {"validation_rows_used": False}:
        raise RuntimeError("LUMIA v3 scientific repair boundary changed")
    if contract.get("persistent_outputs") != PERSISTENT_OUTPUTS:
        raise RuntimeError("LUMIA v3 persistent outputs changed")
...
def validate_kernel(kernel_dir: Path) -> None:
    ...
    if observed != ["kernel-metadata.json", NOTEBOOK_NAME]:
        raise RuntimeError(f"unexpected LUMIA v3 kernel package files: {observed}")
    ...
    for marker in (
        ...
        '[str(VENV_PYTHON), str(RUNNER)]',
        "run_runtime_pilot(model, tokenizer, cohort, runtime, CONFIG)",
        "LUMIA_HIDDEN_STATE_RUNTIME_PILOT_V3 COMPLETE performance_metrics=0 sample_outputs=0",
    ):
        if marker not in code:
            raise RuntimeError(f"generated LUMIA v3 notebook marker missing: {marker}")
    for forbidden in (
        "roc_auc_score",
        "average_precision_score",
        "competition_submit(",
        ...
        if forbidden in code:
            raise RuntimeError(f"forbidden LUMIA v3 runtime code: {forbidden}")
...
def materialize(request_path: Path, research_root: Path, kernel_dir: Path) -> None:
    ...
    builder = research_root / "scripts/build_lumia_hidden_state_runtime_pilot_notebook_v3.py"
    with tempfile.TemporaryDirectory(prefix="lumia-v3-nbformat-shim-") as temp:
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(
            "LUMIA v3 notebook builder failed "
            f"rc={completed.returncode} diagnostic_sha256={hashlib.sha256(diagnostic).hexdigest()}"
        )
    built_dir = research_root / "notebooks/experiments/poisoned-chalice-lumia-hs-runtime-50-v3"
    if kernel_dir.exists():
        shutil.rmtree(kernel_dir)
    shutil.copytree(built_dir, kernel_dir)
    validate_kernel(kernel_dir)
    print(
        "LUMIA_HS_RUNTIME_50_V3_MATERIALIZE PASS "
        "rows=50 max_length=8192 isolated_child=1 live_kernel_mutation=0 "
        "labels_in_scoring=0 metrics=0 sample_outputs=0 gpu=T4 retries=0"
    )
...
def execute(request_path: Path, kernel_dir: Path, kaggle_bin: Path) -> None:
    ...
    if completed.returncode != 0:
        print(
            "LUMIA_HS_RUNTIME_50_V3_AMBIGUOUS_WRITE "
    ...
    print(
        "LUMIA_HS_RUNTIME_50_V3_LAUNCH_ACCEPTED "
...
def main() -> int:
    ...
    if args.static:
        ...
        print("LUMIA_HS_RUNTIME_50_V3_STATIC PASS")
        return 0
    if args.materialize:
