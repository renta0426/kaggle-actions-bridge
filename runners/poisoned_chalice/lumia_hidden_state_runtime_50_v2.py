#!/usr/bin/env python3
"""Validate, materialize, and one-shot launch repaired P1-03 LUMIA 50-row runtime gate."""
...
REQUEST_ID = "20260909-poisoned-chalice-lumia-hs-runtime-50-v2-001"
TARGET = "renta0426/poisoned-chalice-lumia-hs-runtime-50-v2"
RESEARCH_COMMIT = "4f09059ad8bf6eea54a12c377d16c3d6c8127fa6"
...
def validate_request(request: dict) -> None:
    ...
    for key, value in exact.items():
        if request.get(key) != value:
            raise RuntimeError(f"LUMIA runtime v2 request changed: {key}")
    expected_resource = {
        "accelerator": "gpu",
    ...
    if request.get("resource") != expected_resource:
        raise RuntimeError("LUMIA runtime v2 resource contract changed")
    if request.get("api_budget") != {"max_calls": 60}:
        raise RuntimeError("LUMIA runtime v2 API budget changed")
    expected_clean = {
    ...
    if request.get("clean_room") != expected_clean:
        raise RuntimeError("LUMIA runtime v2 clean-room contract changed")
    expected_science = {
    ...
    if request.get("scientific_contract") != expected_science:
        raise RuntimeError("LUMIA runtime v2 scientific contract changed")
...
def validate_research(research_root: Path) -> None:
    paths = {
        ...
        "builder": research_root / "scripts/build_lumia_hidden_state_runtime_pilot_notebook_v2.py",
        "contract": research_root / "configs/p1_03_lumia_hidden_state_runtime_pilot_50_v2_20260909.json",
    }
    ...
    for marker in (
        ...
        if marker not in core:
            raise RuntimeError(f"LUMIA v2 core marker missing: {marker}")
    ...
    for marker in (
        TARGET,
        '"scikit-learn==1.8.0"', '"scipy==1.17.1"', '"joblib==1.5.3"', '"threadpoolctl==3.6.0"',
        "from sklearn.metrics import roc_curve", "from transformers import AutoModelForCausalLM, AutoTokenizer",
        ...
        "LUMIA_HIDDEN_STATE_RUNTIME_PILOT_V2 COMPLETE",
    ):
        if marker not in builder:
            raise RuntimeError(f"LUMIA v2 builder marker missing: {marker}")
    for forbidden in ("roc_auc_score", "average_precision_score", "competition_submit(", "submission.csv", "roc_curve("):
        if forbidden in builder or forbidden in core:
            raise RuntimeError(f"performance/submission code forbidden in runtime v2 gate: {forbidden}")

    if contract.get("task_id") != "P1-03-lumia-hidden-state-runtime-pilot-50-v1":
        raise RuntimeError("LUMIA v2 task identity changed")
    if contract.get("operational_attempt") != "v2_dependency_repair" or contract.get("repair_scope") != "dependency_runtime_only":
        raise RuntimeError("LUMIA v2 repair identity changed")
    if contract.get("status") != "implementation_pending_execution":
        raise RuntimeError("LUMIA v2 contract status changed")
    if contract.get("failed_target") != "renta0426/poisoned-chalice-lumia-hs-runtime-50-v1":
        raise RuntimeError("LUMIA v2 failed-target provenance changed")
    if contract.get("new_target") != TARGET:
        raise RuntimeError("LUMIA v2 target changed")
    if contract.get("failed_target_current_version") != 1 or contract.get("failed_target_terminal_status") != "KERNELWORKERSTATUS.ERROR":
        raise RuntimeError("LUMIA v2 failed-version provenance changed")
    expected_runtime = {
        "numpy": "2.4.2", "pandas": "3.0.1", "scikit-learn": "1.8.0", "scipy": "1.17.1",
        "joblib": "1.5.3", "threadpoolctl": "3.6.0", "transformers": "4.52.0",
        "tokenizers": "0.21.0", "huggingface-hub": "0.30.0",
    }
    if contract.get("author_runtime_alignment_added") != expected_runtime:
        raise RuntimeError("LUMIA v2 repaired runtime changed")
    science = contract.get("scientific_contract_unchanged") or {}
    exact_science = {
        "rows": 50,
        "rows_per_language_label": 5,
        "selection_seed": 20260909,
    ...
    if science != exact_science:
        raise RuntimeError("LUMIA v2 scientific repair boundary changed")
    if contract.get("persistent_outputs") != PERSISTENT_OUTPUTS:
        raise RuntimeError("LUMIA v2 persistent outputs changed")
...
def validate_kernel(kernel_dir: Path) -> None:
    ...
    if observed != ["kernel-metadata.json", NOTEBOOK_NAME]:
        raise RuntimeError(f"unexpected LUMIA v2 kernel package files: {observed}")
    ...
    for marker in (
        ...
        'list(cohort.columns) == ["sample_id", "language", "content"]',
        "run_runtime_pilot(model, tokenizer, cohort, runtime, CONFIG)",
        "LUMIA_HIDDEN_STATE_RUNTIME_PILOT_V2 COMPLETE performance_metrics=0 sample_outputs=0",
    ):
        if marker not in code:
            raise RuntimeError(f"generated LUMIA v2 notebook marker missing: {marker}")
    for forbidden in ("roc_auc_score", "average_precision_score", "competition_submit(", "submission.csv", "roc_curve("):
        if forbidden in code:
            raise RuntimeError(f"forbidden LUMIA v2 runtime code: {forbidden}")
...
def materialize(request_path: Path, research_root: Path, kernel_dir: Path) -> None:
    ...
    builder = research_root / "scripts/build_lumia_hidden_state_runtime_pilot_notebook_v2.py"
    with tempfile.TemporaryDirectory(prefix="lumia-v2-nbformat-shim-") as temp:
    ...
    if completed.returncode != 0:
        ...
        raise RuntimeError(
            "LUMIA runtime v2 notebook builder failed "
            f"rc={completed.returncode} diagnostic_sha256={hashlib.sha256(diagnostic).hexdigest()}"
        )
    built_dir = research_root / "notebooks/experiments/poisoned-chalice-lumia-hs-runtime-50-v2"
    if kernel_dir.exists():
        shutil.rmtree(kernel_dir)
    shutil.copytree(built_dir, kernel_dir)
    validate_kernel(kernel_dir)
    print("LUMIA_HS_RUNTIME_50_V2_MATERIALIZE PASS rows=50 max_length=8192 labels_in_scoring=0 metrics=0 sample_outputs=0 gpu=T4 retries=0 repair=dependency_only")
...
def execute(request_path: Path, kernel_dir: Path, kaggle_bin: Path) -> None:
    ...
    if completed.returncode != 0:
        print(
            "LUMIA_HS_RUNTIME_50_V2_AMBIGUOUS_WRITE "
    ...
    print(
        "LUMIA_HS_RUNTIME_50_V2_LAUNCH_ACCEPTED "
...
def main() -> int:
    ...
    if args.static:
        ...
        print("LUMIA_HS_RUNTIME_50_V2_STATIC PASS")
        return 0
    if args.materialize:
