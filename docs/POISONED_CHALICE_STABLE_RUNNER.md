...
The Min-K++ pilot exposed an avoidable workflow-authoring failure pattern:
...
- all three failures happened before any Kaggle kernel push, so they consumed no Kaggle GPU run and created no Kaggle Notebook version.

The correct Kaggle Acceptable Use Policy URL is `https://www.kaggle.com/aup`. More importantly, policy URLs and launch mechanics must not be copied into every experiment workflow.
...
3. research code at an immutable public research commit referenced by Git blob SHA.
...
- fetches only allowlisted files from the exact public research commit and verifies their Git blobs;
...
**Do not merge a request while this validation is red or absent.** Repair the same PR branch and revalidate there. A failing PR validation is not a reason to merge and probe the protected job.
...
1. reconstructs the already validated bundle from the exact merged commit;
