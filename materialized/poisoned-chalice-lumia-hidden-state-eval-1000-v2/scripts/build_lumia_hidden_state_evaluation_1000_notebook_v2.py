"""Build the repaired P1-03 1,000-row LUMIA hidden-state OOF evaluation Kaggle notebook.

The scientific evaluator remains the already-frozen v1 protocol. This v2 wrapper only
repairs the operational assumption that a Kaggle Notebook output is mounted at a fixed
/kaggle/input path. The exact sealed cache is instead located by its pre-recorded
cache_manifest.json SHA-256, and exactly one match is required.
"""
...
OUT_DIR = ROOT / "notebooks/experiments/poisoned-chalice-lumia-hidden-state-eval-1000-v2"
...
SITE_ROOT = Path("/tmp/lumia-eval-1000-v2-site")
SRC_ROOT = Path("/tmp/lumia-eval-1000-v2-src")
RUNNER = SRC_ROOT / "scripts/run_lumia_hidden_state_probe_evaluation.py"
OUTPUT = Path("/kaggle/working/lumia_hidden_state_evaluation_1000_v2")
INPUT_ROOT = Path("/kaggle/input")
for path in (SITE_ROOT, SRC_ROOT):
    if path.exists():
        shutil.rmtree(path)
if OUTPUT.exists():
    raise RuntimeError("fresh v2 evaluation target must not already contain output")
...
print('LUMIA_EVAL_1000_V2_IMPORT_GATE PASS')
"""
env = os.environ.copy()
...
if completed.returncode != 0:
    raise RuntimeError(f"LUMIA_EVAL_1000_V2_IMPORT_GATE_FAILED rc={completed.returncode}")
# Keep notebook-side public-label materialization on the same pinned stack.
...
# Kaggle may choose a mount directory name for Notebook output inputs that is not
# identical to the source Notebook slug. Never guess that name. Locate the one
# attached cache by exact manifest identity and fail closed on zero/multiple matches.
cache_candidates = []
for candidate_manifest in sorted(INPUT_ROOT.rglob("cache_manifest.json")):
    try:
        ...
        continue
    if candidate_sha256 == EXPECTED_CACHE_MANIFEST_SHA256:
        cache_candidates.append(candidate_manifest)
if len(cache_candidates) != 1:
    raise RuntimeError(
        "exact sealed cache input resolution failed: "
...
if not all(path.is_file() for path in expected_shards):
    raise RuntimeError("exact sealed cache input is missing one or more declared shards")
print(
    "LUMIA_EVAL_1000_V2_CACHE_INPUT_RESOLVED PASS "
    f"manifest_sha256={observed_manifest_sha256} shards=20"
)
...
print(completed.stdout)
if completed.returncode != 0:
    raise RuntimeError(f"LUMIA_EVAL_1000_V2_CHILD_FAILED rc={completed.returncode}")
...
nb.cells = [
    nbformat.v4.new_markdown_cell(
        "# P1-03 LUMIA hidden-state — frozen 1,000-row OOF evaluation (operational repair v2)\n\n"
        "The scientific evaluation is unchanged from the pre-registered v1 contract. "
        "This fresh target repairs only Kaggle Notebook-output input resolution: the sealed cache is located by exact manifest SHA-256 rather than by a guessed mount directory. "
        "Public training membership labels are joined only after cache validation. No validation rows, Public-LB feedback, or competition submission are used."
    ),
...
for index, cell in enumerate(nb.cells):
    cell["id"] = f"lumia-eval-1000-v2-{index:02d}"
